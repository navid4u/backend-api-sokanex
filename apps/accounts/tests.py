...
class AccountsAPITests(APITestCase):
    ...
    def test_logout_rejects_invalid_refresh_without_authentication(self):
        response = self.client.post(
            reverse("logout"),
            {
    ...
    def test_logout_accepts_valid_refresh_with_stale_access_header(self):
        refresh_token = self.authenticate(self.user).data["refresh"]
        self.client.credentials(HTTP_AUTHORIZATION="Bearer expired-or-malformed")
        response = self.client.post(
            reverse("logout"), {"refresh": refresh_token}, format="json"
        )

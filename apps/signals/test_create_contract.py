...
class SignalCreateContractTests(APITestCase):
    ...
    def test_minimal_json_builds_title_and_returns_complete_object(self):
        response = self.client.post("/api/signals/legacy/", self.payload, format="json")
        ...
        mine = self.client.get("/api/signals/legacy/my-signals/")
    ...
    def test_multipart_with_optional_image(self):
        ...
        response = self.client.post("/api/signals/legacy/", payload, format="multipart")
        self.assertEqual(response.status_code, 201)
        self.assertTrue(response.data["image"])

    def test_price_validation_is_farsi_400(self):
        response = self.client.post(
            "/api/signals/legacy/", {**self.payload, "stop_loss": "1.15"}, format="json"
        )
